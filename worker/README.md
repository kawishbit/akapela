...
`aspire run` from the repo root starts this alongside the app on a shared data directory, with its logs in the Aspire Dashboard. To run it on its own instead, against the same data directory the app uses:
