...
- [ ] **Verify `contents: write` actually reaches the `version` job's `git push`.** Same open question as ticket 02 — `default_workflow_permissions: read` at the repo level, workflow declares `contents: write` itself. Should work per GitHub's docs, unverified against this repo's actual enforcement.
- [ ] **Run it once for real.** This cannot be done from here: triggering `workflow_dispatch` pushes a real commit and tag to `main` and, once `build`/`publish` complete, publishes a real GitHub Release with real installers. That's exactly the kind of visible, hard-to-fully-reverse action that needs a human's hand on the trigger, not an agent's. Suggested first run: leave `version` blank (auto — with `02` unmerged there will be no labeled PRs yet, so it'll default to `patch`, producing `v1.0.1`) and confirm the whole chain end to end.
