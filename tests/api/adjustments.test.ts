...
const DEFAULTS = { pitchSemitones: 0, tempoPercent: 100, linked: false, reverbAmount: 0, lowpassHz: 20000 }

describe("a Track's last Adjustments", () => {
  test('a new Track starts at zero semitones, one hundred percent, unlinked, and both Effects bypassed', async () => {
  ...
  test('saving Adjustments returns the Track with them and restores them when the Track is opened again', async () => {
    ...
    const saved = { pitchSemitones: -2, tempoPercent: 90, linked: false, reverbAmount: 25, lowpassHz: 6000 }
  ...
  test('saving a phase-one three-field body defaults both Effects to bypassed', async () => {
    ...
    expect(res.status).toBe(200)
    expect(await res.json()).toMatchObject({ adjustments: { reverbAmount: 0, lowpassHz: 20000 } })

    const reopened = await (await api.get(`/api/tracks/${track.id}`)).json()
    expect(reopened.adjustments).toEqual({ pitchSemitones: -2, tempoPercent: 90, linked: false, reverbAmount: 0, lowpassHz: 20000 })
  })
  ...
  test.each([
    ...
    const track = await importTrack()
    const kept = { pitchSemitones: 4, tempoPercent: 80, linked: false, reverbAmount: 10, lowpassHz: 5000 }
